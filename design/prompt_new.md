...
- `score_manifest` with one stable `instance_id` per visible block in reading order. **Exhaustive block enumeration is mandatory** — see the block-discovery rules below.
- **Block-discovery signals (union, not intersection)**: the walker MUST enumerate every visible block by combining ALL of the following signals at every breakpoint, NEVER only headings/aria-labels:
    1. **Semantic landmarks** — `<header>`, `<footer>`, `<main>`, `<nav>`, `<aside>`, `<article>`, `<section>`, `<form>`, `<figure>`, `<dialog>`, `<details>`, `[role="region" | "banner" | "contentinfo" | "navigation" | "main" | "complementary" | "search" | "form" | "dialog" | "alertdialog" | "status" | "tab" | "tabpanel" | "list" | "listitem"]`, `[aria-label]`, `[aria-labelledby]`, `[aria-describedby]`.
    2. **Heading anchors** — any element containing a `<h1>` / `<h2>` / `<h3>` / `<h4>` / `<h5>` / `<h6>` in reading order.
    3. **Styled-component / BEM class-name families** — regex on `className`: `/(section|wrapper|container|block|panel|band|strip|bar|marquee|ticker|scroller|carousel|slider|announce|promo|cta|hero|footer|header|feature|nav|banner|consent|cookie|toast|snackbar|modal|drawer|sticky|float)/i`. Every element matching the regex whose `getBoundingClientRect()` is wider than 200 px, taller than 8 px, and inside the document flow (not `display: none`, `visibility: hidden`, or `opacity: 0`) MUST be added to the enumeration.
    4. **Vertical-band scan** — walk `document.body` top-to-bottom in 20 px increments; for every y where the topmost visible non-white pixel differs from the surrounding paper background, record the smallest enclosing element whose `getBoundingClientRect()` overlaps that y and whose width ≥ 60 % of `documentElement.clientWidth`. This catches headless decorative strips (tickers, marquees, promo bars, animated backgrounds, dividers with content, quote strips, breadcrumbs).
    5. **Interaction signals** — every `<video>`, `<audio>`, `<canvas>`, `<iframe>`, `<embed>`, `<object>`, `[data-cmp-is]`, `[data-testid]`, `[data-tracking]`, `[data-analytics]`, any element with a bound `wheel`/`scroll`/`animationstart`/`transitionstart` handler, and any element whose computed `animation-name` or `transform` is non-`none` in a keyframe cycle.
    6. **Floating/pinned/overlay signals** — every element with computed `position: fixed | sticky` at any breakpoint (consent bars, chat widgets, back-to-top, sticky nav, marquee-strip on scroll), plus every element with computed `z-index > 0` and a bounding rect overlapping the viewport.
    7. **Repetition signals** — any parent whose direct children share ≥ 80 % class-name prefix and appear ≥ 2 times (list of tiles, logos, testimonials, ticker slides, carousel items, footer columns). Treat the parent as one block; its children join the block's DOM manifest as `count` and per-item rects.
    8. **Common missable patterns (name catalog, must be checked by regex)** — `promo-marquee` / `ticker` / `announcement-bar` / `cookie-consent` / `back-to-top` / `breadcrumb` / `logo-strip` / `stat-row` / `quote-strip` / `divider-with-content` / `pinned-cta` / `newsletter-signup` / `region-selector` / `search-overlay` / `mega-menu-panel` / `skip-link` / `preloader` / `page-progress-bar` / `chat-widget`. If any of these class-name substrings appear on the page but no `score_manifest` entry claims them, the freeze pass STOPs and reports the exact class + rect + a screenshot crop.
- **Coverage proof (mathematical)** — after enumeration, the `score_manifest` MUST satisfy a coverage identity at every breakpoint: the union of block y-ranges (after merging overlaps) covers `[0, document.documentElement.scrollHeight]` with no gap greater than `20 CSS px`. Emit a `coverage_report` table with columns `From y`, `To y`, `Block instance_id` (or `UNCLAIMED`); any row with `Block = UNCLAIMED` and `To y - From y >= 20 px` STOPs the freeze pass. The block owning that y-range MUST be added to the manifest before proceeding.
- Source-section ownership manifest: nearest visual/semantic owner, rect, all direct regions, order, count, role (`featured`, `supporting-list`, `card`, `media`, `controls`, `pagination`, `marquee`, `ticker`, `banner`, `strip`, `overlay`, etc.), prominence, and adjacency.
...
- Responsive/state matrix per block: `static-grid`, `feature-plus-rail`, `horizontal-scroll`, `carousel`, `marquee`, `ticker`, `sticky-strip`, `overlay`; visible order/count, overflow, clipping, controls, pagination, initial/hover/focus/active states, and one complete next/previous (or one full animation cycle for `marquee`/`ticker`) transition when applicable.
...
This gate is mandatory in every input mode and **must always run** in the same turn as the deploy when its external prerequisites are available. Run against the source reference, disabled target, and author target at every breakpoint in the same real-browser session; exported Figma/PDF frames are the browser-loaded source reference when no live page exists. Whenever `SITE_URL` is set, its running page is an additional mandatory acceptance target and cannot be waived by Figma/PDF evidence. The gate cannot be skipped, waived, postponed, or partially reported. If AEM is not running, start it or ask the user to start it. Missing media, missing tokens, failed readiness, or any other implementation gap enters the bounded remediation policy below; an unavailable external prerequisite may terminate only as `BLOCKED` with evidence.

### Bounded Reflection And Retry Policy
...
Component work-item statuses are `PASSED`, `RETRY_QUEUED`, `PASSED_AFTER_REVISIT`, `FAILED_AFTER_RETRY`, and `BLOCKED_COMPONENT`; `RETRY_QUEUED` is never a terminal status. Whole-run terminal statuses are `PASSED`, `COMPLETED_WITH_FAILURES`, and `BLOCKED`. Use `PASSED` when every reachable component is `PASSED` or `PASSED_AFTER_REVISIT`. Use `COMPLETED_WITH_FAILURES` after the retry queue is exhausted when any component is `FAILED_AFTER_RETRY` or `BLOCKED_COMPONENT`. This policy limits execution, not quality: all scoring thresholds and evidence requirements remain unchanged for `PASSED`.
...
- **Maximum author-editability of content**: every visible piece of source content that a business author could plausibly change post-launch MUST be exposed as an AEM authorable field on the component dialog and rendered from the Sling model / HTL binding — never inlined as a literal in HTL, CSS, JS, or a Java constant. The frozen source manifest MUST enumerate every author-editable role per component instance and, for each, declare the field name, dialog tab, widget type (`textfield`, `textarea` with `useFixedInlineToolbar`, `richtext`, `pathfield`, `imagefield` via `cq/dam/gui/coral/components/admin/DamPathField`, `numberfield`, `select` populated by the tokens catalog, `checkbox`, `multifield` for repeatable rows), required/optional flag, default, and validation. Coverage floor per component instance:
    ...
    - **Colors (every color role, mandatory)**: every color role a component paints — section background, section foreground/text color, card background, card text color, eyebrow color, heading color, body color, tag color, border color, CTA background, CTA text, CTA border, divider color, icon color, overlay color, and any other computed color property on a scored role — MUST be an authorable pair of fields:
        1. A `select` field named `<role>Color` (e.g. `bgColor`, `titleColor`, `cardBgColor`, `ctaBgColor`, `ctaFgColor`, `borderColor`, `iconColor`) populated from the site's curated brand palette — every entry is a `{ text: '<Human Label>', value: '<token-key>' }` pair sourced from `--credera-color-*` tokens (e.g. `accent`, `accent-strong`, `text`, `text-inverse`, `paper-bg`, `bg-soft`, `partner-adobe`, `partner-aws`, `partner-salesforce`, `footer-bg`, `insights-bg`). Include a final `other` entry.
        2. A companion `textfield` named `<role>ColorHex` with `fieldLabel="Custom hex (e.g. #FF532C)"`, HTML5 pattern validation `^#([0-9a-fA-F]{3}){1,2}$` (or `^#([0-9a-fA-F]{6}([0-9a-fA-F]{2})?)$` for RGBA hex), placeholder `#RRGGBB`, and `granite:class="cq-dialog-dropdown-showhide-target"` targeting the parent select via `cq-dialog-dropdown-showhide` (target `cq-dialog-checkbox-showhide-target`), so the hex field is **only visible** when the select value is `other`. If the select is anything else, the hex field is hidden and its stored value is ignored.
        3. **HTL binding**: emit an inline CSS custom-property declaration on the component root (or the relevant sub-element root) — for example `style="--cmp-partners-card-bg: ${item.cardBgHex @ context='styleToken'}"` when `select === 'other'`, and `class="cmp-partners__link cmp-partners__link--brand-${item.cardBgKey}"` otherwise. The CSS then reads the custom property with a `token-driven` fallback: `background: var(--cmp-partners-card-bg, var(--credera-color-partner-${key}));`. Never write a hex literal directly into HTL; always route through a CSS custom property so the CSS layer stays literal-free per the tokens contract in section 3.
        4. **Sling model contract**: for every `<role>Color` field the model exposes two accessors — `getRoleColorKey()` returning the select value (`String`) and `getRoleColorHex()` returning the sanitized hex or `null`. Sanitisation must reject anything that does not match the hex regex above (return `null` on invalid) so authors cannot inject CSS via the hex field. In HTL, use `@ context='styleToken'` (or a project-provided XSS filter) to further protect the property value at render.
        5. **Coverage**: this rule applies to every existing and every new component. A component that paints a color role via a hard-coded CSS token reference (e.g. `background: var(--credera-color-accent);` without an authoring override path) fails this rule and must be reworked so the CSS reads `var(--cmp-<component>-<role>, var(--credera-color-<fallback>));` and the dialog exposes the pair. Add or extend the color select each time a new role is introduced — never leave a color choice to CSS alone.
        6. **Evidence**: the authorability matrix must include one row per color role per component with columns `Role`, `Select Value`, `Hex Value`, `Hidden Correctly When Not "other"?`, and `Rendered CSS Custom Property Value On Live Target`. Any missing role, any missing hex field, any hex field that shows up without `other` selected, or any component still using a raw token reference where an author-driven override is missing, hard-fails the component and blocks it from being marked `PASSED`.
...
For `PASSED` and `COMPLETED_WITH_FAILURES`, include the following at the top of the final response. For `BLOCKED`, include every item captured before the global blocker and mark unavailable items `NOT_MEASURED` with the blocking prerequisite; never fabricate evidence:

1. Per-instance score table at every breakpoint, with each axis citing source and target evidence AND the per-component `visualMatchPercent` from real side-by-side screenshots taken this turn.
2. Component-type minima table (each cell = `min(visualMatchPercent, propertyScore, authorabilityScore)` — never an average).
3. Breakpoint page composites.
4. Cross-breakpoint minimum instance, component type, and page composite.
5. **Retry Revisit Table** — one row per queued `(component, breakpoint)` with `Score After Attempt 3`, `Score Before Revisit`, `Score After Revisit`, `Score Delta`, `Change Made`, and final work-item `Status`. A missing row or an unmeasured score delta invalidates `PASSED` or `COMPLETED_WITH_FAILURES` when the retry queue was nonempty.
6. **Side-by-side visual-parity artefacts** — for every component at every breakpoint, publish the paths to `evidence/<component>-<bp>-source.png`, `evidence/<component>-<bp>-target.png`, `evidence/<component>-<bp>-diff.png`, and `evidence/<component>-<bp>-mask.png`, and the numeric `visualMatchPercent`. Also publish `evidence/full-<bp>-source.png` and `evidence/full-<bp>-target.png` per breakpoint. A completion output without these artefacts is invalid.
7. **Per-Component Width Match Table** — one row per component per breakpoint with `Source Width (CSS px)`, `Target Width (CSS px)`, `Delta`, `Source Full-Bleed?`, `Target Full-Bleed?`, and `Status` (`PASS` iff `|Delta| <= 1 CSS px`, matching full-bleed flag, and matching horizontal offset). Widths MUST be pulled from real `getBoundingClientRect().width` on both the live source page and the deployed AEM target page in the same turn. A completion output that omits this table, or reports a component `PASS` while its width row is `FAIL`, is invalid.
8. **Coverage Report Table** — per breakpoint, sorted merged block y-ranges covering `[0, documentElement.scrollHeight]` with columns `From y`, `To y`, `Block instance_id` (or `UNCLAIMED`), `Class Chain`, and `Discovery Signal(s)`. A completion output with any `UNCLAIMED >= 20 CSS px` row, or that omits this table, is invalid. The `Discovery Signal(s)` column MUST show that at least one of the eight discovery signals from Section 1 caught the block — a manifest that only ever cites "heading" as the discovery signal is a defect (headings alone miss headless decorative strips like `promo-marquee`, `announcement-bar`, `logo-strip`).
9. Exactly one terminal status line, always drawn from real measurements taken in the current turn:
...
VISUAL PARITY GATE: COMPLETED_WITH_FAILURES at <breakpoints> — attempted <N> work items, passed <P>, failed <F>, blocked components <B> — minimum measured instance <score>% (required >95%)
VISUAL PARITY GATE: BLOCKED after <N> prerequisite attempts — prerequisite: <exact unavailable dependency> — resume action: <required user or environment action>
```

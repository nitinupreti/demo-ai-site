...
This gate is mandatory in every input mode. Run against the source reference, disabled target, and author target at every breakpoint in the same real-browser session; exported Figma/PDF frames are the browser-loaded source reference when no live page exists. Whenever `SITE_URL` is set, its running page is an additional mandatory acceptance target and cannot be waived by Figma/PDF evidence. A missing AEM instance defers, but never waives, the gate.
...
Every raw instance score, component-type minimum, and page composite must be strictly `>85%` at every breakpoint. `85.000%` fails; compare before display rounding. A high average cannot hide a failure.
...
5. Exactly one terminal status line:

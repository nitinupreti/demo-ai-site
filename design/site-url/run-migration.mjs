...
Options:
  -u, --url <url>              Live source URL; prompted when omitted
      ...
      --aem-port <port>        Local AEM author port (default: 4504)
...
async function promptForSiteUrl() {
  if (!process.stdin.isTTY) {
    throw new Error('Provide the source URL with --url when input is not interactive.');
  }
  const terminal = readline.createInterface({ input: process.stdin, output: process.stdout });
  try {
    return (await terminal.question('Live site URL: ')).trim();
  } finally {
    terminal.close();
  }
}
...
async function main() {
  ...
  options.siteUrl = normalizeSiteUrl(options.siteUrl || await promptForSiteUrl());

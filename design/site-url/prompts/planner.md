...
4. **Ownership.** The exact files each component may write. Keep scopes disjoint and minimal:
   its own component directory, its own Sling Model(s), its own SCSS/CSS/JS partial, **and its own
   unit test path**. A component that owns anything under `src/main/java` must also own a matching
   path under `src/test/java` — its role requires a unit test, and the ownership guard rejects any
   write outside the paths you grant. Prefer one test class per component
   (`.../src/test/java/<package>/<Name>ModelTest.java`) over a shared test class: two components
   editing one test file is a merge conflict the orchestrator will refuse.
...
- gives any component a shared path (`/conf/**`, page content, `filter.xml`, `clientlib-base|site`,
  shared SCSS, `components/page/**`, `pom.xml`);

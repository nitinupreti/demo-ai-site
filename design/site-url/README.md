...
- Local AEM author is running. The launcher asks for its port and offers `4504` when you press Enter.
...
Double-click `run-migration.cmd`, paste the live page URL, enter the local AEM author port (or press Enter for `4504`), and continue. The window displays stage messages, assistant updates, and tool activity. On successful completion it opens the generated AEM page and leaves all evidence under `design/scratch/migration-<run_id>/`.
...
Non-interactive runs must supply both `--url` and `--aem-port` explicitly.
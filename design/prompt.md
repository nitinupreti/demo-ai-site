# DESIGN_SOURCE
FIGMA_URL:   "https://www.figma.com/design/esrkZjAKSEtb83ys9cQM8A/Travel-Website-Landing-Page--Community-?node-id=108-84&t=L5xga41lZQCNh8Ua-0"
...
- **Model fields:** superset both designs. Fields used by only one style keep sensible `@Default`s and are guarded in HTL by `data-sly-test="${model.style == '<key>'}"`. Multifield children may also carry a per-item `style` field when the same list renders differently between designs.
- **Dialog:** add a `Design Style` select at the top of the Style tab with all supported keys and a `selected` default matching the first design; label design-specific fields with a `(<style-key>)` suffix in `fieldLabel` so authors know which style consumes them; use `fieldDescription` when the intent isn't obvious.

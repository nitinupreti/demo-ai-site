...
$tokenResp = Invoke-WebRequest -Uri "$Base/libs/granite/csrf/token.json" -Headers $hdr -UseBasicParsing
$csrf = ($tokenResp.Content | ConvertFrom-Json).token
$hdr['CSRF-Token'] = $csrf

foreach ($p in @('/content/dam/demo-ai-site','/content/dam/demo-ai-site/design','/content/dam/demo-ai-site/design/figma')) {
  try {
    $body = "./jcr:primaryType=sling:Folder"
    Invoke-WebRequest -Uri "$Base$p" -Method POST -Headers $hdr -Body $body -ContentType 'application/x-www-form-urlencoded' -UseBasicParsing -ErrorAction SilentlyContinue | Out-Null
  } catch {}
}
...
foreach ($f in $files) {
  ...
  try {
    $u = "$Base$dstFolder.createasset.html"
